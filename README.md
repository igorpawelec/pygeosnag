...
7. **Crowns.** `grow_crowns` converts the mode's RGB-like trio to CIELAB and runs pygeoadaptels' `grow_seeds` with weights (0.5, 2.5, 1.0), a Delta-E tolerance of 15, a radius of 20 px and hole filling — the recipe worked out on a spruce plot with bleached snags. Since 0.3.4 it works tile by tile around the points (a halo of two radii keeps neighbouring crowns competing), never reading the raster whole: a 796-megapixel orthophoto with 12 000 points runs in bounded memory with no temporary file, where the first version read 9.5 GB and died writing a CIELAB copy to the temp drive. Crowns from a tiled run differ from a whole-raster run only by tie-breaking at crown boundaries (median 1.7 m² per affected crown, total area within 3%). One MultiPolygon per point, `adaptel_id` = the point's index; `--workers` grows tiles in parallel from the command line. `--space ndvi_L` grows on 100·NDVI and CIELAB lightness instead of the weighted CIELAB: on a benchmark of 1200 verified crowns from 8 sites it reached a median IoU of 0.62 against 0.53 for the shipped recipe, with the same over-segmentation; it needs a NIR band, so the default stays `lab_w`.
